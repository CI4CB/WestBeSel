...
python /code/westbesel/manage.py collectstatic --noinput
python /code/westbesel/manage.py syncdb --noinput
python /code/westbesel/manage.py makemigrations
python /code/westbesel/manage.py migrate --noinput
...
if [ "$DJANGO_PRODUCTION" != "true" ]; then
    ...
    if [ "$ROOT_PASSWORD" == "" ]; then
        ...
        echo "from ConfigParser import SafeConfigParser; parser = SafeConfigParser(); parser.read('/code/config.ini'); from django.contrib.auth.models import User; print 'Root user already exists' if User.objects.filter(username='root') else User.objects.create_superuser('root', 'admin@example.com', parser.get('general', 'ROOT_PASSWORD'))" | python /code/westbesel/manage.py shell
    else
        # Root password environment variable IS set; so, use it
        echo "import os; from django.contrib.auth.models import User; print 'Root user already exists' if User.objects.filter(username='root') else User.objects.create_superuser('root', 'admin@example.com', os.environ['ROOT_PASSWORD'])" | python /code/westbesel/manage.py shell
    fi
else
